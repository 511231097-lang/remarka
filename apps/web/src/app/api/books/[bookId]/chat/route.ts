...
import { LITERARY_SECTION_KEYS, toBookChatMessageDTO, type BookChatEntryContextDTO, type LiterarySectionKeyDTO } from "@/lib/books";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!session) {
    session = await prisma.bookChatSession.create({
      data: {
        bookId,
        userId: authUser.id,
        title: "Новый чат",
  ...
  await prisma.bookChatSession.update({
    ...
    data: {
      ...
      title: session.title === "Новый чат" ? message.slice(0, 80) : session.title,
    },
  });
  ...
  await prisma.bookChatMessage.update({
    ...
    data: {
      citationsJson: finalizedPayload as unknown as Prisma.InputJsonValue,
    },
  });
  ...
  return NextResponse.json({
    sessionId: session.id,
    message: toBookChatMessageDTO({
      ...assistant,
      citationsJson: finalizedPayload as unknown as Prisma.JsonValue,
    }),
  });
}
