...
const GENERAL_PROMPTS = [
  "Почему главный герой поступает именно так?",
  "Какие эпизоды лучше всего раскрывают главную идею книги?",
  "Что в этой книге важнее всего под поверхностью сюжета?",
];
    ...
    async function loadInitialSessions() {
      try {
        ...
        setCurrentSessionId((current) =>
          current && nextSessions.some((session) => session.id === current) ? current : nextSessions[0]?.id || null
        );
      } catch (error) {
        if (!active) return;
        setBookError(error instanceof Error ? error.message : "Не удалось загрузить чат");
                  ...
                  >
                    ...
                    {message.role === "assistant" ? (
                      <>
                        ...
                        <ChatEvidence evidence={message.evidence} maxItems={4} />
                      </>
                    ) : null}
            ...
            <div className="border-t border-border pt-6">
              <div className="flex gap-3">
                <textarea
                  ...
                  className="flex-1 px-5 py-4 bg-card border border-border rounded-2xl resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 text-foreground placeholder:text-muted-foreground"
                  rows={3}
                ...
                <button
                  ...
                  className="px-8 py-4 bg-primary text-primary-foreground rounded-2xl hover:opacity-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 self-end"
                >
