...
const LEGAL_DOCS: Record<LegalKey, {
  ...
  terms: {
    ...
    sections: [
      ...
      { h: "4. Учётная запись", b: "Для доступа к приватным функциям (библиотека, загрузка, чат) нужна учётная запись. Вход выполняется через Google или Yandex ID — по вашему выбору. Вы отвечаете за конфиденциальность доступа к учётной записи выбранного провайдера. Вы можете удалить учётную запись и все связанные с ней данные через профиль в любой момент." },
  ...
  privacy: {
    ...
    sections: [
      ...
      { h: "6. Передача третьим лицам", b: "Для оказания услуги мы привлекаем следующих обработчиков: (а) провайдер авторизации по выбору пользователя — Google LLC (США) при выборе входа через Google либо ООО «ЯНДЕКС» (Российская Федерация) при выборе входа через Yandex ID; передаются имя, e-mail и идентификатор учётной записи; (б) Google LLC (США) через Google Vertex AI — отправка фрагментов вашего файла, текста запроса и истории чата для генерации ответа ассистентом; согласно действующим на дату публикации настоящей Политики условиям Google Cloud Service Terms, передаваемые данные не используются Google для обучения публичных моделей; возможна автоматическая проверка контента на злоупотребления, при которой подозрительные запросы могут логироваться сроком до 90 дней; Исполнитель отслеживает изменения этих условий и в случае их существенного изменения уведомит пользователей; (в) ООО НКО «ЮMoney» (Российская Федерация) — приём платежей через сервис ЮKassa; (г) поставщики хостинга и инфраструктурных услуг на территории Российской Федерации — размещение базы данных и файлового хранилища; (д) ООО «ЯНДЕКС» через сервис Yandex SmartCaptcha (Российская Федерация) — защита публичных форм от автоматизированных обращений; в момент проверки передаются IP-адрес и технические данные браузера. Данные передаются только в объёме, необходимом для оказания услуги." },
      { h: "7. Трансграничная передача персональных данных", b: "Часть обрабатываемых данных передаётся за пределы Российской Федерации, в США, при использовании Google Vertex AI (для всех пользователей сервиса) и Google OAuth (для пользователей, выбравших авторизацию через Google). При выборе авторизации через Yandex ID трансграничной передачи персональных данных на этапе авторизации не происходит. США не входят в перечень государств, обеспечивающих адекватную защиту прав субъектов персональных данных, утверждённый Роскомнадзором. Передача осуществляется с вашего явного согласия, которое вы даёте при регистрации в сервисе. Уведомление о трансграничной передаче подано в Роскомнадзор в порядке ч. 3 ст. 12 закона № 152-ФЗ." },
