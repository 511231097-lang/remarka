...
//     передаче — без этого фразы про РКН остаются формально декларативными.
//  3. Перепроверить дату редакции при следующем существенном изменении.
const LEGAL_DOCS: Record<LegalKey, {
  ...
  terms: {
    ...
    sections: [
      ...
      { h: "3. Возрастное ограничение", b: "Сервис предназначен для пользователей старше 18 лет. Подтверждая согласие при регистрации, вы заявляете, что достигли этого возраста. Это ограничение установлено как требованиями применимого законодательства, так и условиями подрядчиков, обеспечивающих работу ассистента." },
      ...
      { h: "10. Доступность сервиса", b: "Исполнитель стремится к стабильной работе, но не гарантирует непрерывной доступности. Плановые обслуживания, сбои инфраструктуры и обстоятельства непреодолимой силы могут приводить к временной недоступности отдельных функций. Гарантированный SLA для тарифов начального уровня не предоставляется." },
  ...
  privacy: {
    ...
    sections: [
      ...
      { h: "6. Передача третьим лицам", b: "Для оказания услуги мы привлекаем следующих обработчиков: (а) Google LLC (США) — авторизация через Google OAuth, передаются имя, e-mail и идентификатор аккаунта; (б) Google LLC (США) через Google Vertex AI — отправка фрагментов вашего файла, текста запроса и истории чата для генерации ответа ассистентом; согласно условиям Google Cloud, переданные данные не используются Google для обучения моделей; возможна автоматическая проверка контента на злоупотребления, при которой подозрительные запросы могут логироваться сроком до 90 дней; (в) ООО НКО «ЮMoney» (Российская Федерация) — приём платежей через сервис ЮKassa; (г) поставщики хостинга и инфраструктурных услуг на территории Российской Федерации — размещение базы данных и файлового хранилища. Данные передаются только в объёме, необходимом для оказания услуги." },
      ...
      { h: "12. Возрастные ограничения", b: "Сервис предназначен для лиц старше 18 лет и не рассчитан на несовершеннолетних. Если вы обнаружили, что ребёнок зарегистрировался самостоятельно, сообщите на privacy@remarka.app — мы удалим учётную запись." },
